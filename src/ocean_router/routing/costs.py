...
class CostContext:
    ...
    def move_cost(self, y: int, x: int, lat: float, prev_bearing: Optional[float], move_bearing: float, min_depth: float, weights: CostWeights) -> float:
        step_cost = move_cost_nm(self.grid_dx, self.grid_dy, lat)
        penalty = 0.0
        penalty += self.bathy.depth_penalty(y, x, min_depth, weights.near_shore_depth_penalty)
        if self.tss:
    ...
    def blocked(self, y: int, x: int, min_depth: float) -> bool:
        if not self.bathy.is_safe(y, x, min_depth):
            return True
