...
def main():
    ...
    parser.add_argument("--grid", type=Path, default=Path("configs/grid_0.5nm.json"))
    ...
    # Load masks
    land_mask = load_mask(args.data_dir / "land/land_mask_05nm.npy")
    
    lane_mask = sepzone_mask = sepboundary_mask = None
    tss_dir = args.data_dir / "tss"
    if (tss_dir / "tss_lane_mask_05nm.npy").exists():
        lane_mask = load_mask(tss_dir / "tss_lane_mask_05nm.npy")
    if (tss_dir / "tss_sepzone_mask_05nm.npy").exists():
        sepzone_mask = load_mask(tss_dir / "tss_sepzone_mask_05nm.npy")
    if (tss_dir / "tss_sepboundary_mask_05nm.npy").exists():
        sepboundary_mask = load_mask(tss_dir / "tss_sepboundary_mask_05nm.npy")
